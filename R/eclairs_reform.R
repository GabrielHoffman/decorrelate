...
#'
#' @details Reform the dataset from the SVD, drop features, then recompute the SVD.  If the original SVD was truncated, then the reconstruction of the original data will be approximate.  Note that the target shrinkage matrix is the same as in \code{Sigma.eclairs}, so \eqn{\nu} is not recomputed from the retained features. 
#'
