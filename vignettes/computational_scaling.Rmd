...
```{r define.function}
eval_runTime = function(df_values, naive.maxp=10000){
...
values = as.integer(sort(c(	seq(500, 20000, length.out=50), 
		...
		seq(200000, 1000000, length.out=5))))

# values = as.integer(sort(c(	seq(500, 10000, length.out=20))))

df_values = data.frame(n = 1000, p = values, k = 50)

df_time = eval_runTime( df_values, 10000 )
...
fig = ggplot(res$df_observed, aes(p, value, color=variable)) + geom_point() + theme_classic(16) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="bottom") + xlab("Number of features (p)") + ylab("Time (seconds)") + geom_line(data=res$df_smoothed, aes(p, value, color=variable)) + scale_color_manual(name="Method", values = c("red", "blue", "green")) + ggtitle("n = 1000")
...
```{r increase.n}
values = as.integer(sort(c(	seq(200, 10000, length.out=50))))
p = 1000

df_values = data.frame(n = values, p=p, k = 100)

df_time = eval_runTime( df_values, p+1  )
...
fig = ggplot(res$df_observed, aes(n, value, color=variable)) + geom_point() + theme_classic(16) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="bottom") + xlab("Number of samples") + ylab("Time in seconds") + geom_line(data=res$df_smoothed, aes(n, value, color=variable)) + scale_color_manual(name="Method", values = c("red", "blue", "green")) + ggtitle("p = 3000") + geom_vline(xintercept=3000, linetype="dashed", colr="grey40")
