...
# due to parallel processing across 48 cores, curves are not exactly cubic and linear
values = c(seq(100, 12000, length.out=50), seq(15000, 1000000, length.out=15))
...
fig1 = ggplot(df_melt, aes(p, value, color=variable)) + geom_point() + theme_bw(16) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + xlab("Number of features") + ylab("Time in seconds") + geom_line(data=df_melt_p, aes(p, value, color=variable)) + scale_color_manual(name="Method", values = c("red", "blue")) 

fig1 + xlim(0, 10000) + ylim(0, max(df_time$naive, na.rm=TRUE))
