...
---
title: "Fast Probabilistic Whitening Transformation for Ultra-High Dimensional Data"
#subtitle: ''
...
```{r run.examples, fig.height=2.5, fig.width=10, fig.cap ="**Figure 1: Intuition for data whitening transformation**. **A)** Original data, **B)** Data rotated along principal components, **C)** Data rotated and scaled, **D)** Data rotated, scaled and rotated back to original axes.", echo=FALSE}
library(decorrelate)
library(mvtnorm)
library(ggplot2)
library(cowplot)
...
plot_data = function(Y, rotation, rank, lim){
  ...
  ggplot(df, aes(X1, X2, color=rank)) + 
    ...
    xlim(lim) + 
    ylim(lim) + 
    geom_segment(x=0, y=0, xend=H[1,1], yend=H[2,1], color="#38df37", arrow = arrow(length = unit(0.03, "npc")), size=1) + 
    geom_segment(x=0, y=0, xend=H[1,2], yend=H[2,2], color="#38df37", arrow = arrow(length = unit(0.03, "npc")), size=1)
}


fig1 = plot_data(Y, rotation = 1, rank(value), lim) + ggtitle(TeX('$Y$'))
fig2 = plot_data( Y, rotation = 2, rank(value), lim) + ggtitle(TeX('$YU$'))
fig3 = plot_data( Y, rotation = 3, rank(value), lim) + ggtitle(TeX('$YU\\Lambda^{-\\frac{1}{2}}$'))
fig4 = plot_data(Y, rotation = 4, rank(value), lim) + ggtitle(TeX('$YU\\Lambda^{-\\frac{1}{2}}U^T$'))
