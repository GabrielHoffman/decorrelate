...
out2 = adjclust:::matL(Csq, h=5)

head(out2)

p = nrow(Csq)

out3 = sparseMatrix(i=1, j=1, x=0, dims=c(p, h+1))
...
for(i in seq_len(p)){
  ...
  out3[i,] = Csq[i,idx]
                    ...
                    if( i == 12508 & k < 50){
                      Rcpp::Rcout << j << ' '<< k << ' ' << 2*value << std::endl;
                    }

library(inline)
matL <- rcpp(signature(Csq_ = "dsCMatrix", h_ = "integer", p_="integer"),
               " 
               arma::SpMat<double> Csq = as< arma::SpMat<double> >(Csq_);
              
              int p = Rcpp::as< int>(p_);
               int h = Rcpp::as< int>(h_);
               rma::SpMat<double> out(p, h+1);
...
system.time({
out <- matL(Csq, as.integer(h), as.integer(ncol(Csq)))
})

system.time({
out2 <- adjclust:::matL(Csq, h=h)
})

mean(abs(out-out2))
as.numeric(object.size(out)) / as.numeric(object.size(out2))



matR <- rcpp(signature(Csq_ = "dsCMatrix", h_ = "integer", p_="integer"),
               " 
               arma::SpMat<double> Csq = as< arma::SpMat<double> >(Csq_);
              ...
              int k;
               ...
               for( int i=0; i<p; i++){
                k = 0;
                for( int j=i; j>std::max(i-h-1, (int) 0); j--){
...
out <- matR(Csq, as.integer(h), as.integer(ncol(Csq)))

out2 <- adjclust:::matR(Csq, h=h)
