...
df.standard = data.frame(res.standard$x)
df.standard = merge(df.standard, gtex_sample, by.x="row.names", by.y="SAMPID")

ggplot(df.standard, aes(PC1, PC2, color=SMTSD)) + geom_point() + theme_classic() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5))
# ggplot(df.standard, aes(PC2, PC3, color=SMTSD)) + geom_point() + theme_classic() + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5))
```


# decorrelate
```{r decorrelate}
res.decorr = pca_cor2( t(gtex_log2cpm), 100, maxit=2000)
```
...
kmax = 10
score.list = data.frame(k = 1:kmax)

score.list$standard = sapply( 1:kmax, function(k){
  concordance(df.standard[,-1][,1:k], df.standard$SMTSD)
})

score.list$decorrelate = sapply( 1:kmax, function(k){
  concordance(res.decorr$U[,1:k], df.standard$SMTSD)
})
